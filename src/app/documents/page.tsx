    ...
    <div className="px-6 py-6 w-full flex flex-col h-[calc(100vh-64px)] gap-6 ">
      <div className="flex justify-between">
        <div className="flex items-center">
          <p className="text-2xl font-semibold tracking-tight pr-3 border-r-2 border-[#E3E8EF]">
            My Documents
          </p>
          <div className="text-[#757575] text-sm pl-3">
            Total {MOCK_DATA.length}
          </div>
        </div>
        <UploadBtn />
      </div>
      <div className="flex-1 flex border-[1px] border-[#D9D9D9] rounded-[12px]">
        {MOCK_DATA.length > 0 ? (
...
async function getData(): Promise<AppDocument[]> {
  // Fetch data from your API here.
  return MOCK_DATA.map((doc) => ({
    id: doc.id,
